...
# 2. 最近 10 筆匹配紀錄
print("\n=== 最近 10 筆匹配紀錄（AD 像素檢查）===")
rows2 = query_d1("SELECT tag, pixel_id, matched, timestamp FROM clicks WHERE matched = 1 ORDER BY timestamp DESC LIMIT 10")
for r in rows2:
    tag = r.get('tag','')
    pixel = str(r.get('pixel_id',''))[:15] if r.get('pixel_id') else '(空)'
    time = r.get('timestamp','')
    status = 'OK' if r.get('pixel_id') else 'MISSING'
    print(f"  {tag:5s} | pixel={pixel:18s} | {status:7s} | {time}")

# 3. 修復前後比較
print("\n=== 修復前後比較（Worker 部署後）===")
rows3 = query_d1("""
SELECT 
  CASE WHEN timestamp >= '2026-03-23T08:30:00' THEN '修復後' ELSE '修復前' END as period,
...
FROM clicks 
WHERE matched = 1 AND timestamp >= '2026-03-23T00:00:00'
GROUP BY period
""")
for r in rows3:
    period = r.get('period','')
...
# 4. BC 像素檢查 - Worker 裡寫死的 BC pixel
print("\n=== BC 像素（783186198187359）回傳邏輯 ===")
print("  BC 像素寫死在 Worker 的 sendBcEvent 函數中")
print("  回傳自定義事件：{TAG_PREFIX}_{EventName} + ALL_{EventName}")
print("  觸發點：")
print("    - /bc-event GET/POST → PageView, Contact, Purchase")
print("    - LINE 跳轉 302 → Lead")
print("  BC 像素不在 ad_config DataTable 中（type=bc 僅供記錄）")
print("  BC 像素不受 Config API 冷啟動影響（寫死在 Worker 中）")
