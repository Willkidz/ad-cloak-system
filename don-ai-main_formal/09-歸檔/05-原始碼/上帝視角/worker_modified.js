...
var FALLBACK_MASTER_PIXEL_MAP = {
  ...
  "sz": { pixel: "1701026614201171", token: "EAAeahovhP0cBQ7DLruPWR3fcDRZAnvWCPi9SiFcA90cX5kcAZCxVcNFHth0VrAwBoItuK6kq10f9jJV7U45HxV4zEOC8hVpSrjScxYMQTngUHAaHZCW9lQEHLEEaOe0q40brrGffiyLkS8Wt7w8h1993PoxWt9PXqnQZB7ViXUBH548eNcWWRZBZBZCfVwvfejqbQZDZD" }
};
// BC 像素的預設值（冷啟動時使用）
var FALLBACK_BC_PIXEL = { pixel: "783186198187359", token: "EAAeahovhP0cBQ7DLruPWR3fcDRZAnvWCPi9SiFcA90cX5kcAZCxVcNFHth0VrAwBoItuK6kq10f9jJV7U45HxV4zEOC8hVpSrjScxYMQTngUHAaHZCW9lQEHLEEaOe0q40brrGffiyLkS8Wt7w8h1993PoxWt9PXqnQZB7ViXUBH548eNcWWRZBZBZCfVwvfejqbQZDZD" };
var FALLBACK_CONFIG = {
...
async function sendBcEvent(eventName, productPrefix, userData, bcPixel) {
  ...
  if (!bcPixel || !bcPixel.pixel || !bcPixel.token) {
    return; // BC 像素資訊不完整，不發送
  }
  var url = "https://graph.facebook.com/v21.0/" + bcPixel.pixel + "/events?access_token=" + bcPixel.token;
  try {
    await fetch(url, {
      method: "POST",
...
var index_default = {
  async fetch(request, env, ctx) {
    ...
    ctx.waitUntil(
      env.DB.prepare(
        "INSERT INTO clicks (click_id,timestamp,tag,ad_code,line_oa_id,ip_address,user_agent,accept_language,ip_country,ip_asn,fbclid,fbc,fbp,pixel_id,capi_token,pixels,destination,cf_colo,tls_version,http_protocol,referer) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
      ).bind(
        clickId,
        timestamp,
        tag,
