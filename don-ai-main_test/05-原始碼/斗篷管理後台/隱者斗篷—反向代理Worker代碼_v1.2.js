...
function validateReferer(request) {
  ...
  // 直接訪問（無 Referer）- 視為可信
  if (!referer) {
    return { isValid: true, reason: "direct_access", source: "direct" };
  }
  ...
  try {
    ...
    // 檢查是否來自信任的廣告來源
    for (const trustedSource of TRUSTED_AD_SOURCES) {
      if (refererHost === trustedSource || refererHost.endsWith("." + trustedSource)) {
        return { isValid: true, reason: "trusted_ad_source", source: trustedSource };
      }
    }

    // 檢查是否來自自身域名（內部轉跳）
    const currentHost = request.headers.get("Host") || "";
    if (refererHost === currentHost || refererHost.endsWith("." + currentHost)) {
      return { isValid: true, reason: "internal_referral", source: currentHost };
    }

    // 其他來源視為可疑
    return { isValid: false, reason: "suspicious_referer", source: refererHost };
  } catch (e) {
    // Referer 格式不合法
    return { isValid: false, reason: "invalid_referer_format", source: referer.substring(0, 50) };
  }
}
...
async function handleRequest(request) {
  ...
  if (url.pathname === "/_debug") {
    ...
    const refererValidation = validateReferer(request);
    const cf = request.cf || {};
    return new Response(JSON.stringify({
      detection,
      referer_validation: refererValidation,
      info: {
  ...
  // 正常用戶進行 Referer 驗證（A5）
  const refererValidation = validateReferer(request);
  
  // 如果 Referer 驗證失敗，視為可疑流量，路由到安全頁
