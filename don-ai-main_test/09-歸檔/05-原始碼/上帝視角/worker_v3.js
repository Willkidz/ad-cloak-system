// line-redirect Worker v3 - 非阻塞 Config + 多像素
...
// v3: 非阻塞 — 有快取用快取，沒快取用 fallback，背景更新
function getConfigInstant() {
  const now = Date.now();
...
export default {
  async fetch(request, env, ctx) {
    ...
    // v3: 瞬間取得 config，不等待 API
    const config = getConfigInstant() || FALLBACK_CONFIG;
